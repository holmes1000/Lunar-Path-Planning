...
sys.path.append(os.path.abspath("torchvision_helpers"))

from torchvision_helpers.engine import train_one_epoch, evaluate
import torchvision_helpers.utils
import torchvision_helpers.transforms as T
...
class CraterDataset(object):
    def __init__(self, root, transforms):
        ...
        print(f"Found {len(self.imgs)} images and {len(self.annots)} annotations in {self.root}")
    ...
    def __getitem__(self, idx):
        ...
        print(f"Processing image: {img_path}, annotation: {annot_path}")
        ...
        if os.path.getsize(annot_path) != 0:
            ...
            print(f"Found {len(bboxs)} bounding boxes in {annot_path}")
            num_objs = len(bboxs)
        ...
        if self.transforms is not None:
            ...
            sample = self.transforms(
                                    image=img,
                                    bboxes=target['boxes'].tolist(),  # convert tensor to list of boxes
                                    labels=labels.tolist()            # convert tensor to list of labels
                                )
        # img = sample['image']
...
if __name__ == "__main__":
    ...
    # Safe: only looks at metadata, won't initialize CUDA runtime
    device = torch.device('cuda') if (torch.version.cuda is not None) else torch.device('cpu')
    print("Using device:", device, " torch.version.cuda =", torch.version.cuda)

    # our dataset has two classes only - background and crater
    num_classes = 2

    dataset_test = CraterDataset('martianlunar-crater-detection-dataset\\test', get_transform(train=False))
